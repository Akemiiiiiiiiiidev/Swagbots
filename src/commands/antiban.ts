...
export const antiban: Command = {
  ...
  async execute(interaction) {
    ...
    if (sub === "ver") {
      ...
      if (!roleId) {
        await interaction.reply(containerReplyOrganized(["# **ANTIBAN**", `Nenhum cargo antiban configurado. Use /antiban setup.`], { ephemeral: true }));
        return;
      }
      await interaction.reply(
        containerReplyList(`ANTIBAN — Configuração`, [
          { label: `Cargo protegido`, items: [`<@&${roleId}>`, "Membros com este cargo nao podem ser banidos pelo bot."] },
        ])
      );
      return;
    }
    ...
    if (sub === "setup") {
      ...
      await interaction.reply(
        containerReplyList(`ANTIBAN — Cargo configurado`, [
          { label: `Cargo`, items: [`<@&${role.id}>`] },
          { label: `Detalhes`, items: [`Configurado por: <@${interaction.user.id}>`, "Membros com este cargo nao poderao ser banidos pelo bot."] },
        ])
      );
      return;
    }
    ...
    if (sub === "remover") {
      if (!getAntibanRoleId(guild.id)) {
        await interaction.reply(containerReplyOrganized([`Nenhum cargo antiban configurado.`], { ephemeral: true }));
        return;
      }
      clearAntiban(guild.id);
      await interaction.reply(
        containerReplyList(`ANTIBAN — Desativado`, [
          { label: `Detalhes`, items: [`Removido por: <@${interaction.user.id}>`, "O sistema de antiban foi desativado."] },
        ])
      );
    }
  },
