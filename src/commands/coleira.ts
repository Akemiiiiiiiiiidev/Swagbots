  ...
  async execute(interaction) {
    ...
    if (sub === "ativar") {
      ...
      if (targetUser.id === interaction.user.id) {
        await interaction.reply(containerReplyOrganized([`Voce nao pode colocar coleira em si mesmo.`], { ephemeral: true }));
        return;
      }
      if (targetUser.bot) {
        await interaction.reply(containerReplyOrganized([`Nao e possivel colocar coleira em bots.`], { ephemeral: true }));
        return;
      }
      const target = await fetchGuildMember(guild, targetUser.id);
      if (!target) {
        await interaction.reply(containerReplyOrganized([`Este usuario nao esta no servidor.`], { ephemeral: true }));
        return;
      }
      if (hasColeira(guild.id, target.id)) {
        ...
        await interaction.reply(
          containerReplyOrganized(
            [
              `<@${target.id}> ja possui coleira ativa${isMine ? " por voce" : ` por <@${currentExecutorId}>`}.`,
              `Use \`/coleira remover\` para remover primeiro.`,
            ],
            { ephemeral: true }
          )
        );
      ...
      await interaction.reply(
        containerReplyOrganized([
          ...
          movedNow
            ? `**Movido agora:** Sim — para **${targetVoice!.name}**`
            : targetVoice
            ? `**Aviso:** Voce nao esta em call — entre em uma call para ser movido.`
            : `**Aviso:** O alvo nao esta em call no momento.`,
          `Sempre que <@${target.id}> mudar de canal de voz, voce sera movido junto automaticamente.`,
        ])
      );
      return;
    }
    ...
    if (sub === "remover") {
      ...
      if (!hasColeira(guild.id, targetUser.id)) {
        await interaction.reply(containerReplyOrganized([`<@${targetUser.id}> nao possui coleira ativa.`], { ephemeral: true }));
        return;
      }
      const currentExecutorId = getColeira(guild.id, targetUser.id)!;
      if (currentExecutorId !== interaction.user.id) {
        await interaction.reply(
          containerReplyOrganized([`Essa coleira foi ativada por <@${currentExecutorId}>. Somente quem ativou pode remover.`], { ephemeral: true })
        );
        return;
      }
    ...
    if (sub === "remover-todas") {
      ...
      if (list.length === 0) {
        await interaction.reply(containerReplyOrganized([`Voce nao possui nenhuma coleira ativa.`], { ephemeral: true }));
        return;
      }
      ...
      await interaction.reply(
        containerReplyOrganized([
          ...
          [
            ...
            `**Total removidas:** ${list.length}`,
            `**usuarios :** ${list.map((id) => `<@${id}>`).join(", ")}`,
          ].join("\n"),
        ])
      );
      return;
    ...
    if (sub === "listar") {
      ...
      if (all.length === 0) {
        await interaction.reply(containerReplyOrganized([`Nenhuma coleira ativa neste servidor.`], { ephemeral: true }));
        return;
      }
      const lines = all
        .map(({ targetId, executorId }) => `<@${targetId}> → seguido por <@${executorId}>`)
        .join("\n");
      await interaction.reply(
        containerReplyOrganized([
          "# **COLEIRA**",
          [`**Total ativo:** ${all.length}`, "", lines].join("\n"),
        ])
      );
    }
  },
