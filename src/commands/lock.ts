...
export const lock: Command = {
  ...
  async execute(interaction) {
    ...
    if (isChannelLocked(channel, guild)) { await interaction.reply(containerReplyOrganized([`O canal <#${channel.id}> ja esta travado.`], { ephemeral: true })); return; }
    const motivo = interaction.options.getString("motivo") ?? "Sem motivo informado";
    await lockChannel(channel, guild);
    await interaction.reply(
      containerReplyOrganized([
        "# **LOCK**",
        [`**Chat travado**`, `**Canal:** <#${channel.id}>`, `**Motivo:** ${motivo}`, `**Moderador:** <@${interaction.user.id}>`, `**Data:** <t:${Math.floor(Date.now() / 1000)}:F>`].join("\n"),
      ])
    );
  },
};
