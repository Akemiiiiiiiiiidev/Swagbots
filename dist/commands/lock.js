...
exports.lock = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addChannelOption((o) => o.setName("canal").setDescription("Canal para travar").addChannelTypes(discord_js_1.ChannelType.GuildText, discord_js_1.ChannelType.GuildAnnouncement).setRequired(false))
    ...
    async execute(interaction) {
        ...
        if ((0, channelLock_1.isChannelLocked)(channel, guild)) {
            await interaction.reply((0, container_1.containerReplyOrganized)([`${container_1.E} O canal <#${channel.id}> ja esta travado.`], { ephemeral: true }));
            return;
        }
        const motivo = interaction.options.getString("motivo") ?? "Sem motivo informado";
        await (0, channelLock_1.lockChannel)(channel, guild);
        await interaction.reply((0, container_1.containerReplyOrganized)([
            "# **LOCK**",
            [`${container_1.V} **Chat travado**`, `${container_1.E} **Canal:** <#${channel.id}>`, `${container_1.E} **Motivo:** ${motivo}`, `${container_1.E} **Moderador:** <@${interaction.user.id}>`, `${container_1.E} **Data:** <t:${Math.floor(Date.now() / 1000)}:F>`].join("\n"),
        ]));
    },
};
